...
export const sendEmailNotification = async (
  ...
  try {
    // If SendGrid API key is not configured, skip sending
    if (!process.env.SENDGRID_API_KEY) {
      console.log('SendGrid API 키가 설정되지 않아 이메일 전송을 건너뜁니다.');
      return false;
    }

    const msg = {
      to: email,
      from: process.env.SENDGRID_FROM_EMAIL || 'noreply@mond.io.kr', // Must be verified in SendGrid
