...
// Date formatting function (from INICIS template)
...
async function testV2DirectBilling() {
  ...
  try {
    console.log('\n=== TESTING INICIS V2 DIRECT BILLING API ===\n');
    ...
    // INICIS V2 Configuration (from template)
    ...
    // Create data object (matching INICIS template exactly)
    const data = {
        url: "www.mond.io.kr",
        ...
        billKey: billingKeyData.billingKey  // Using the StdPay billing key
    };

    console.log('\nTest Payment Details:');
    console.log('Order ID (moid):', data.moid);
    console.log('Amount:', data.price, 'KRW');
    console.log('Timestamp:', timestamp);
    console.log('BillKey:', data.billKey);

    // Hash Encryption (exactly as in INICIS template)
    ...
    // Request options (exactly as in INICIS template)
    let options = {
        mid: mid,
        type: type,
        paymethod: paymethod,
